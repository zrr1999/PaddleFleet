...
class StandardMLPSharedExpert(MLP):
    ...
    def forward(self, hidden_states: paddle.Tensor) -> paddle.Tensor:
        # E-456: gated one-shot dump of MTP-shared fc1 X. Off unless
        # MODEL_REPRO_MTP_SHARED_SWIGLU_DUMP=1. Detach+write only; no
        # register_hook (hooks have moved enorm on this graph).
        ...
        if _swiglu_dump and _live:
            ...
            _li = str(getattr(self, "_shared_layer_no", "-"))
            _key = f"fwd_{_li}_{_rk}"
            ...
            if _key not in type(self)._e456_dumped:
                ...
                if _x.dtype != paddle.float32:
                    _x = _x.cast("float32")
                _x.cpu().numpy().tofile(
                    os.path.join(_live, f"paddle_mtpsh_fc1x_l{_li}_r{_rk}.f32.bin")
                )
                with open(
                    os.path.join(_live, f"paddle_mtpsh_fc1x_l{_li}_r{_rk}.json"),
                    "w",
                ) as _f:
                    _f.write(
                        '{"layer":"%s","rank":%d,"shape":%s,"dtype":"%s"}\n'
                        % (_li, _rk, list(hidden_states.shape), str(hidden_states.dtype))
                    )
